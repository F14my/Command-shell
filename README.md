...
#### Особенности rm:
- Чтобы избежать перезаписи при удалении файлов с одинаковыми именами, используется автоматическое переименование. При перемещении файл получает уникальное имя по следующей схеме:
```<TIMESTAMP>_<HASH>_<original_name>```, где:

* TIMESTAMP - время удаления в миллисекундах,
* HASH - короткий хэш от абсолютного пути файла,
